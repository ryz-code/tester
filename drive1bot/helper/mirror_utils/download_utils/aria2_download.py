...
from drive1bot.helper.mirror_utils.status_utils.aria_download_status import AriaDownloadStatus
...
class AriaDownloadHelper(DownloadHelper):
    ...
    def __onDownloadStarted(self, api, gid):
        log.info(f"onDownloadStart: {gid}")
        update_all_messages()

    def __onDownloadComplete(self, api: API, gid):
        log.info(f"onDownloadComplete: {gid}")
    ...
    def __onDownloadError(self, api, gid):
        ...
        log.info(f"onDownloadError: {gid}")
        dl = getDownloadByGid(gid)
        download = api.get_download(gid)
        error = download.error_message
        log.info(f"Download Error: {error}")
